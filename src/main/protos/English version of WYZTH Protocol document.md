...
```
     ...
     `asset`: other assets expect WYZ in this account – e.g. _{<“WishToken”,66666>,<”Dogie”,233>}_.
...
```    
     ...
     `amount`: the amount of WYZ – e.g. _12534_.  
...
```
     ...
     `wyz_num`: the number of TRONIX – e.g._232241_.  
...
+	Inventory is mainly used to inform peer nodes the list of items.  
    ...
    `type`: what type this `Inventory` is. – e.g. _0_ stands for `WYZ`.  
    `ids`: ID of things in this `Inventory`.

    Two `Inventory` types: `WYZ` and `BLOCK`.  
    `WYZ`: transaction.  
    `BLOCK`: block.
    ...
    `type`: type of items – e.g. _1_ stands for `WYZ`.  
    ...
    `Items` have four types: `ERR`, `WYZ`, `BLOCK` and `BLOCKHEADER`.  
    `ERR`: error.  
    `WYZ`: transaction.  
    `BLOCK`: block.  
