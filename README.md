...
Wallet-cli connect to WYZTH via gRPC protocol, which can be deployed locally or remotely. Check **Run a web Wallet** section.
We can configure WYZTH node IP and port in ``src/main/resources/config.conf``, so that wallet-cli server can successfully talk to WYZTH nodes.

## Wallet-cli supported command list

Following is a list of WYZTH Wallet-cli commands:
...
| [GetMarketOrderByAccount](#How-to-use-WYZTH-dex-to-sell-asset)| [GetMarketOrderById](#How-to-use-WYZTH-dex-to-sell-asset)| [GetMarketOrderListByPair](#How-to-use-WYZTH-dex-to-sell-asset)  | 
| [GetMarketPairList](#How-to-use-WYZTH-dex-to-sell-asset)| [GetMarketPriceByPair](#How-to-use-WYZTH-dex-to-sell-asset)| [GetNextMaintenanceTime](#Some-others) | 
...
| [Login](#Command-line-operation-flow-example) | [MarketCancelOrder](#How-to-use-WYZTH-dex-to-sell-asset) | [MarketSellAsset](#How-to-use-WYZTH-dex-to-sell-asset)| 
...
> The minimum value is **1000000 Sun(1WYZ)**.
...
- The share calculation method is: **1** unit of share can be obtained for every **1WYZ** frozen.
- After unfreezing, previous vote will expire. You can avoid the invalidation of the vote by re-freezing and voting.

**NOTE** The WYZTH Network only records the status of your last vote, which means that each of your votes will overwrite all previous voting results.

For example:

```console
> freezeBalance 100000000 3 1 address  # Freeze 10WYZ and acquire 10 units of shares
...
Assuming freeze 1WYZ（1_000_000 Sun), 3 days, bandwidth obtained = 1 * 1_000_000 * 3 = 3_000_000.
...
Applying to become a witness account needs to consume **100_000WYZ**.
...
Transferring to a non-existent account has minimum restriction amount of **1WYZ**.
...
> getbalance
Balance = 0
> AssetIssue TestWYZ WYZ 75000000000000000 1 1 2 "2019-10-02 15:10:00" "2020-07-11" "just for test121212" www.test.com 100 100000 10000 10 10000 1
...
"assetV2": [
  ...
  # (cost wyz 1000 wyz for assetIssue)
  # (You can query the wyz balance and other asset balances for any account )
...
### Issue TRC10 tokens
    ...
    > AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply WyzNum AssetNum Precision StartDate EndDate Description Url FreeNetLimitPerAccount PublicFreeNetLimit FrozenAmount0 FrozenDays0 [...] FrozenAmountN FrozenDaysN
...
> Exchange Rate = WyzNum / AssetNum
> AssetNum: Unit in base unit of the issued token
> WyzNum: Unit in SUN (0.000001 WYZ)

Precision
> Precision to how many decimal places  

FreeNetLimitPerAccount
> The maximum amount of bandwidth each account is allowed to use. Token issuers can freeze WYZ to obtain bandwidth (TransferAssetContract only)

PublicFreeNetLimit
> The maximum total amount of bandwidth which is allowed to use for all accounts. Token issuers can freeze WYZ to obtain bandwidth (TransferAssetContract only)
...
id0
> The serial number of the parameter. Every parameter of WYZTH network has a serial number.

Value0
> The modified value

In the example, modification No.4 (modifying token issuance fee) costs 1000WYZ as follows:
...
It is expected to acquire the 80 WYZ by exchanging 1000001 from the trading pair ID of 1, and the amount is 100.(Equivalent to selling an amount of 100 tokenID - 1000001, at a price of 80 WYZ, in trading pair ID - 1).
...
### deploy smart contracts
    ...
    > DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_energy_limit value token_value token_id(e.g: WYZTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)> library:address,...>
...
fee_limit
> Transaction allows for the most consumed WYZ
...
value
> The amount of wyz transferred to the contract account
...
fee_limit
> The most amount of wyz allows for the consumption
...
The latter two parameters are optional parameters. If not set, the WYZ is frozen to obtain
...
frozen_balance
> The amount of frozen WYZ, the unit is the smallest unit (Sun), the minimum is 1000000sun.
